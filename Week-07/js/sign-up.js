...
var validateForm = function (form) {
    ...
    },
    {
      ...
      isValid: function (input) {
        ...
        if (input.value) {
          ...
          if (street.length < 5) {
            this.errorMessage =
              "Direction street must have more than 5 letters.";
            return false;
          }
          if (
            number.length < 2 ||
            number.length > 4 ||
            !hasOnlyNumbers(number)
          ) {
  ...
  var validateFormGroup = function (formGroup) {
    ...
    for (var i = 0; i < inputsValidations.length; i++) {
      if (
        input.getAttribute("id") === inputsValidations[i].inputForm &&
        !inputsValidations[i].isValid(input)
      ) {
  ...
  formElement.addEventListener("submit", function (e) {
    ...
    for (var i = 0; i < allForms.length; i++) {
      ...
      if (!error) {
        fieldErrors.push(
          allForms[i].querySelector(".form-input").getAttribute("id")
        );
      }
    }
    if (fieldErrors.length !== 0) {
      ...
      var repeatedPasswordValue =
        document.querySelector("#repeated-password").value;
